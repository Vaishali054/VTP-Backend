...
export const buyStock = async (req, res) => {
    ...
    try {
        ...
        let userStock = await UserStocks.findOne({ User_Id: userId, Company_Id: company.Company_Id });
        ...
        res.status(200).json({ message: 'Stock purchased successfully', transaction: newTransaction });
    } catch (error) {
        console.error(error);
...
export const sellStock = async (req, res) => {
    try {
        ...
        const stocks = await UserStocks.findOne({ User_Id: userId, Company_Id: company.Company_Id });
        console.log(stocks);
        if (!stocks) {
        ...
        if (stocks.numberOfStocks < quantity) {
            return res.status(404).json({ error: 'Not enough stocks in possession!' });
        }
        ...
        if (stocks.quantity == 0) {
            await UserStocks.findOneAndDelete({ User_Id: userId, Company_Id: company.Company_Id });
        }
        else {
            await stocks.save();
        }

        res.status(200).json({ message: 'Stock sold successfully', transaction: newTransaction });
    } catch (error) {
        console.error(error);
        res.status(500).json({ error: 'Internal server error' });
    }
